...
if __name__ == "__main__":
    ...
    try:
        import os
        account_type = os.environ["ig_account_type"]
        if(account_type == "Real"):
            account_type = True
            print("Account Type is Real")
        else:
            print("Account Type is Demo (Default account)")
            account_type = False

        user_name = os.environ["ig_user_name"]
        password = os.environ["ig_password"]
        account_id = os.environ["ig_account_id"]
        api_key = os.environ["ig_api_key"]
    except Exception:
        print("Ensure account_type, user_name, password, account_name environment variables are set. Eg:")
        print("account_type: Demo/Real")
    ...
    subscription = Subscription(
        mode="MERGE",
        #can be a list of items[MARKET:IX.D.FTSE.CFD.IP, ]
        items=["MARKET:IX.D.FTSE.CFD.IP"],
        #fields=["stock_name", "last_price", "time", "bid", "ask"],
        fields=["BID", "OFFER", "HIGH", "LOW"], #Krishna
        adapter="")
    ...
    def on_item_update(item_update):
        ...
        print("Name:{}, Bid:{}, Offer:{}, High:{}, Low: {}".format(
            item_update['name'], 
            item_update['values']['BID'],
            item_update['values']['OFFER'],
            item_update['values']['HIGH'],
            item_update['values']['LOW']))
